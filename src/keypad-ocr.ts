...
function horizontalSegments(grid: boolean[][], bbox: ReturnType<typeof getBBox>, relY: number): number {
  const y = bbox.minY + Math.floor(bbox.height * relY);
  if (y < 0 || y >= grid.length) return 0;
  let segments = 0, inBright = false;
  for (let x = bbox.minX; x <= bbox.maxX; x++) {
    if (grid[y][x] && !inBright) { segments++; inBright = true; }
    else if (!grid[y][x]) { inBright = false; }
  }
  return segments;
}
...
interface Features {
  ...
  seg25: number; seg50: number; seg75: number;
}

function extractFeatures(grid: boolean[][]): Features {
...
function classifyDigit(f: Features): string {
  // 2 holes → 8
  ...
  if (f.holes === 1) {
    ...
    // 0: 상중하 모두 갭 있음
    if (hasGap25 && hasGap50 && hasGap75) return "0";

    // 6: 하단에 구멍 → 상단 갭 없고 하단 갭 있음
    if (!hasGap25 && hasGap50 && hasGap75) return "6";
    if (!hasGap25 && !hasGap50 && hasGap75) return "6";

    // 4 vs 9: 상단 갭 있고 하단 갭 없음
    if (hasGap25 && hasGap50 && !hasGap75) {
      // 4: fill 낮음, tl 밀도 낮음 / 9: fill 높음, tl 밀도 높음
      if (f.fillRatio < 0.47) return "4";
      return "9";
    }

    // fallback: cy로 구분 (9: 상단 편중, 6: 하단 편중)
    if (f.cy < 0.47) return "9";
    if (f.cy > 0.48) return "6";

    // 추가 fallback
    if (f.tl > f.bl) return "9";
    return "6";
  }
  ...
  if (f.fillRatio < 0.42) {
    ...
    if (f.cy < 0.45) return "7";
    return "1";
  }

  // 2, 3, 5: fill ≥ 0.42
  // 3: cx 우측 편중 (cx > 0.53)
  if (f.cx > 0.53) return "3";

  // 2 vs 5: 사분면 패턴으로 구분
  // 5: tl 밀도 높고 br 밀도 높음 (왼쪽 위 + 오른쪽 아래)
  // 2: tr 밀도 높고 bl 밀도 높음 (오른쪽 위 + 왼쪽 아래)
  if (f.tl > f.tr && f.br > f.bl) return "5";
  if (f.tr > f.tl && f.bl > f.br) return "2";

  // fallback: cy로
  if (f.cy < 0.48) return "5"; // 5는 약간 상단 편중
  return "2";
}
