...
export async function login() {
    ...
    if (runtime.headless && !credentials) {
        throw new Error(`headless/OpenClaw 환경에서는 수동 로그인 창을 띄울 수 없습니다. ${sessionDir}/credentials.json 을 먼저 채우거나 DISPLAY/COUPANG_HEADLESS=0 환경에서 다시 실행해주세요.`);
    }
    await withBrowser(async (page, context) => {
        ...
        if (!loggedIn) {
            if (runtime.headless) {
                throw new Error("저장된 계정 정보로 자동 로그인하지 못했습니다. headless/OpenClaw 환경에서는 수동 로그인으로 전환할 수 없으니 credentials.json을 확인하거나 GUI 환경에서 다시 실행해주세요.");
            }
